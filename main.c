...
Window* window;
TextLayer *title_layer, *location_layer, *description_layer, *temperature_layer, *time_layer;

char location_buffer[64], description_buffer[64], temperature_buffer[32], time_buffer[32];
...
void process_tuple(Tuple *t)
{
	...
	switch(key) {
		...
		case KEY_DESCRIPTION:
			...
			snprintf(description_buffer, sizeof("Description: couldbereallylongname"), "%s", string_value);
			text_layer_set_text(description_layer, (char*) &description_buffer);
			break;
		case KEY_TEMPERATURE:
			//Temperature received
			snprintf(temperature_buffer, sizeof("Temperature: XX \u00B0C"), "Temp: %d \u00B0C", value);
	...
	strftime(time_buffer, sizeof("Last updated: XX:XX"), "Updated: %H:%M", tm);
...
void window_load(Window *window)
{
	...
	description_layer = init_text_layer(GRect(5, 60, 144, 30), GColorBlack, GColorClear, "RESOURCE_ID_GOTHIC_18", GTextAlignmentLeft);
	text_layer_set_text(description_layer, "Description: N/A");
	layer_add_child(window_get_root_layer(window), text_layer_get_layer(description_layer));
	
	temperature_layer = init_text_layer(GRect(5, 90, 144, 30), GColorBlack, GColorClear, "RESOURCE_ID_GOTHIC_18", GTextAlignmentLeft);
